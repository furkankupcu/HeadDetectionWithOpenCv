...
def dlipToN(bulunanNoktalar,maske):
...
#nok = numpyCevir(simetriAl(arrayAdd(dlipToN(omuzGezerDaireviLimitli(img, kafaBul(img), yonSol, 60),img),dlipToN(omuzGezerDaireviLimitli(img, kafaBul(img), yonSag, 60),img))))
#fitEllipse(nok,img)

def kafaTespit():

        img = np.load("sonuc_np/004.jpg.npy")
        height, width = img.shape[0], img.shape[1]
        ortX = kafaBul(img).y
        baslangıc=height-ortX

        for limit in range(baslangıc+20,height,3):
            print(limit)
            ...
            widthE = ellipse[1][0]
            heightE = ellipse[1][1]
            print(widthE,heightE)
            oran = round(heightE/widthE,2)
            print(oran)

            if  oran > 1.20 and oran < 1.50:
