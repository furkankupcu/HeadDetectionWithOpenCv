...
img=np.load("sonuc_np/005.jpeg.npy")
...
def kafaBul(maske:np.array): #kafanın orta noktasını bulma
    height, width = maske.shape[0], maske.shape[1]
...
def numpyCevir(noktalar:list): #array'i numpy array'a çevirir
...
def dlipToN(bulunanNoktalar,maske): #dlip'ten arraya
...
def fitEllipse(noktalar,img):
    ...
    print(widthE,heightE,heightE/widthE)
...
def simetriAl(noktalar): #son noktaya göre simetri alır
...
nok = numpyCevir(simetriAl(arrayAdd(dlipToN(omuzGezerDaireviLimitli(img, kafaBul(img), yonSol, 80),img),dlipToN(omuzGezerDaireviLimitli(img, kafaBul(img), yonSag, 80),img))))
fitEllipse(nok,img)
